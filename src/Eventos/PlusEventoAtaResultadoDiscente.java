...
public class PlusEventoAtaResultadoDiscente extends EventosPadrao {
	...
	protected EventosAta evento;

	public PlusEventoAtaResultadoDiscente(final EventosAta evento) {
		this.evento = evento;
		this.main = evento.getMain();
	...
	protected MouseListener onClickRowTable = new MouseListener() {
		...
		public void mouseClicked(final MouseEvent e) {
			if (e.getClickCount() == 2) {
				try {
					final Aluno aluno = evento.modeloArquivo.getContato(tabela.getSelectedRow()).getAluno();
					evento.processoMostarAluno(aluno);
					finalizeOperation();
				} catch (final Exception ex) {
					JOptionPane.showMessageDialog(null,
